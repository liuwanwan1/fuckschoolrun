...
LSPatch 免 Root Xposed 模块，向目标 App（支付宝阳光校园跑）注入模拟加速度计、计步器和步检测器数据。
...
│ 支付宝 (LSPatch 修补后)                                  │
│                                                          │
│  阳光校园跑 → SensorManager.getDefaultSensor(TYPE_STEP)  │
│       │                                                  │
│       ├─ 设备有计步器 → 返回真实Sensor                    │
│       └─ 设备无计步器 → 降级返回加速度计Sensor ─────────┐ │
│                                                          │ │
│  阳光校园跑 → SensorManager.registerListener(...)        │ │
│       │                                                  │ │
│       └─ LspatchStepModule 拦截 ─────────────────────┐   │ │
│           • 缓存 Sensor 对象引用                      │   │ │
│           • 记录 listener → 目标传感器类型映射        │   │ │
