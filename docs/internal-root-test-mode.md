...
- 配置驱动的 Root 能力开关架构，配置文件位于应用私有目录 `root_feature_config.json`，Root 面板支持手动热重载
- Root管理器、su路径、Hook框架包名的环境检测
- 开发者选项、当前应用模拟位置授权、旧版模拟位置全局开关检测
- Root隐藏迹象的启发式提示：检测到Root管理器但未发现常见su路径时提示环境不一致
- 本次测试会话二次确认
- 显式 `su -c id` 授权探测
- 本地审计日志使用 Android Keystore AES-GCM 加密写入应用私有 `root_test_audit.log.enc`，XLog 仅输出审计摘要哈希
- GM 沙箱测试接口占位：仅基于当前路线在内存中生成 `FOR TESTING ONLY` 轨迹、速度、距离预览，不写入系统服务
- Debug 内测压力测试平台入口：传感器压力场景、GPX/NMEA 回放和环境攻击面演示均只生成 `FOR TESTING ONLY` 报告

## Root能力开关与热重载

Root能力配置以每个功能一个开关的方式保存，当前开关包括：
...
- GM沙箱测试接口
- Frida应用级动态注入请求
- Root NMEA注入、基站/WiFi信号模拟、模拟位置检测绕过、目标应用Hook、系统服务数据流、系统传感器注入

热重载只重新读取配置并刷新面板状态。受限能力即使开关开启，也只记录“已请求但被安全边界阻断”的状态，不执行 attach/spawn/hook、不修改目标应用返回值、不写入系统服务。
...
- 读取、篡改系统核心服务数据流
- 注入系统传感器事件
- SafetyNet / Play Integrity 检测规避、隐藏Root或隐藏Hook框架

## Android 9-13兼容边界

Root面板在 Android 9-13 目标范围内仅使用公开应用 API、应用私有文件、Android Keystore 和显式 `su -c id` 授权探测。为降低误触发系统完整性检测的风险，当前实现不包含 Frida attach/spawn、Zygote 注入、系统服务替换、SafetyNet/Play Integrity 绕过或隐藏 Root 痕迹逻辑。
